 ...
 *
 * An announced collection has no dials to show, so it is a plain link with the
 * status beside it — a panel with nothing in it would be worse than no panel.
 */
export function CollectionMenu({
  ...
  const href = path(lang, `/collections/${collection.slug}`);
  ...
  const link = (
    <Link
      href={href}
      aria-expanded={collection.products.length > 0 ? open : undefined}
      aria-current={active ? "page" : undefined}
      className={`u-focus u-link flex items-center gap-2 py-6 text-[0.7rem] uppercase tracking-[0.28em] transition-colors duration-300 ${
        active ? "text-text" : "text-muted hover:text-text"
      }`}
    >
      {collection.name}
      {collection.status === "announced" && (
        <span className="rounded-full border border-line px-2 py-0.5 text-[0.55rem] tracking-[0.2em] text-faint">
          {t("comingSoon")}
        </span>
      )}
    </Link>
  ...
  if (collection.products.length === 0) return link;
